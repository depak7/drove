...
A local engineering pipeline for people who want to delegate code without delegating judgment.
Describe a feature, approve one plan, then come back to an isolated, reviewed branch, a pushed
compare link, and evidence you can inspect before merging.
...
The point is **independent review**: the harness that wrote the code never grades its own
homework. The reviewer starts fresh and sees what a human reviewer should see: the diff and the
stated intent, not the implementer's private reasoning.
